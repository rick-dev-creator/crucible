...
public readonly struct ChainResult<T>
{
    ...
    private ChainResult(T? value, IReadOnlyList<IError>? errors, Exception? exception, IReadOnlyList<IDomainEvent> events)
    {
        _value = value;
        _errors = errors;
    ...
    public static ChainResult<T> DomainFailure(IReadOnlyList<IError> errors, IReadOnlyList<IDomainEvent> events)
    ...
    public ChainResult<T> Catch(Func<Exception, IReadOnlyList<IError>> handler)
    {
